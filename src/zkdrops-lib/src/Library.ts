...
export function toHex(bi: bigint): string {
    // Force bi to be treated as unsigned 256-bit integer
    if (bi < 0n) {
...
async function generateCircuitInputJson(
    ...
    let commitment = await poseidon2(nullifier.valueOf(), secret.valueOf());
    let mp = mt.getMerkleProof(commitment);
    let nullifierHash = await poseidon1(nullifier.valueOf());
    let inputObj = {
        root: mt.root.val,
